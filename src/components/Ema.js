...
function Stock({
  ...
  const fetchStock = () => {
    ...
    const API_Call = `https://www.alphavantage.co/query?function=EMA&symbol=${symbol}&interval=daily&time_period=10&series_type=open&apikey=${API_KEY}`;
    ...
    fetch(API_Call)
      ...
      .then(data => {
        for (const key in data['Technical Analysis: EMA']) {
          stockChartXValuesFunction.push(key);
          stockChartYValuesFunction.push(data['Technical Analysis: EMA'][key]['EMA']);
  ...
  return (
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}><h1>{symbol}</h1>
