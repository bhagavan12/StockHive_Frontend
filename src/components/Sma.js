...
function Stock({
  ...
  const fetchStock = () => {
    ...
    const API_Call = `https://www.alphavantage.co/query?function=SMA&symbol=${symbol}&interval=daily&time_period=10&series_type=open&apikey=${API_KEY}`;
    ...
    fetch(API_Call)
      ...
      .then(data => {
        for (const key in data['Technical Analysis: SMA']) {
          stockChartXValuesFunction.push(key);
          stockChartYValuesFunction.push(data['Technical Analysis: SMA'][key]['SMA']);
